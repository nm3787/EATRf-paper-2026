...
def analyze(indicess, quiet=False):
    ...
    for i in range(len(barriers)):
        ...
        if not quiet:
            ...
            max_biases = [np.max(traj[:,1]+barr_add) for traj in data]
...
if not args.bootstrap:
    logk0_best, gamma_best, logk0_opes = analyze([list(range(len(data))) for data in datas], quiet=args.quiet)
    ...
    for i in range(args.numboots):
        ...
        logk0, gamma, logk0_opesf = analyze(indicess, quiet=True)
